(ns indrive.main
  "Kotodama WASM entrypoint for the inDrive clean-room actor (L4) — Clojure port.
  ...
  facts (`indrive.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "indrive")
(def tier "L4")
...
(def entity-specs
  [{:entity "Driver"   :plural "drivers"   :id-prefix "indrive_dri"
   ...
   {:entity "Rating"   :plural "ratings"   :id-prefix "indrive_rat"
...
(defn emit-facts
  "EAVT facts for one record: {\"indrive.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "inDrive-compat" :tier tier :entities entities} 200])
...
(defn start! [] :inDrive-compat/ready)
